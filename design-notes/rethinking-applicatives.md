...
It is possible to define custom operation for F# computations expressions that permit applicative syntax.  For custom
operators it is often important to appreciate the difference between the "composition" or "build time" of the nodes in the applicative
and the "runtime" of the applicative.

In the example below I show:

* A logging operation "logAtBuildtime" that is executed during **composition** of the applicative. Such a custom opertion does **not** use `ProjectionParameter` and does **not** have access to the variables in scope.
...
let reader1 =
    reader { 
        ...
        logAtBuildtime "building!"
        checkAtRuntime (a > b)
        logAtBuildtime "building more!"
...
module Reader = 
    ...
    let logAtBuildtime msg (r: Reader<'T>) = 
        printfn "%s" msg
        r
...
type ReaderBuilder() = 
    ...
    [<CustomOperation("logAtBuildtime", MaintainsVariableSpaceUsingBind = true) >]
    member _.LogAtBuildtime(x: Reader<'T>, msg: string) =
         Reader.logAtBuildtime msg x
