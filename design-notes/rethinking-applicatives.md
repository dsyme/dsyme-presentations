...
## Appendix: Using custom operations with `let! ... and! ...`

It is possible to define custom operation for F# computations expressions that permit applicative syntax, for example
a custom operation "check" that does additional validation and uses a `ProjectionParameter` to access the variables in scope:
```fsharp
let res1 =
    result { 
        let! a = resultValue1 
...
```
The library code necessary is below. There is [another sample for logging in the F# test code](https://github.com/dotnet/fsharp/blob/3f9172ada35b6db08f64ee592785879d60ac6cb9/tests/fsharp/Compiler/Conformance/DataExpressions/ComputationExpressions.fs#L161).  

However, there are important limitations, in particular the CE **must** define `Bind` and `Return` - see the section above for the considerations with doing this.  In the above case this is essential as the custom operation has access to the variables
in scope via `Bind`.

Likewise you can write custom operations that perform transformations of the whole computation "from the outside" rather than accessing
the defined variable space - asalways these do **not** use `ProjectionParameter`. For example, [this gist](https://gist.github.com/dsyme/5dd08c84ad7bdee3ad556af742172eda) shows how to define named node types where the `prefix` custom
operator adds a prefix to the name of the node being defined:
```fsharp
let test1() = 
    node { 
        let! v1 = tip "a" 3
...
test1() // gives "test1/a,b,c"
```
In this second case, `Bind` and `Return` must still be defined - it seems as though in this case this restriction could be
lifted, as there is no access to the variable space.

Here is the library code for the first custom operation `check` above.
