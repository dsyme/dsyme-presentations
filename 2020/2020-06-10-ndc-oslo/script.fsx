...
module Functional = 
    ...
    let s7a = c7 |> List.map (fun x -> printfn "map (1), x = %d" x; x * 10) 

    // Transform again
    let s7b = s7a |> List.map (fun x -> printfn "map (2), x = %d" x; x * 10) 
    ...
    compute [ 1 .. 10000 ]
    compute ([ 1 .. 10000 ] |> List.filter (fun x -> x <> 5))
